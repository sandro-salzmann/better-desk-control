...
use crate::protocol::{dir_cmd, POLL};
...
impl DeskController {
    ...
    /// (event-driven UIs). `direction` is `"up"` or `"down"`.
    pub async fn start_hold(self: &Arc<Self>, direction: &str) {
        let cmd = dir_cmd(direction);
        self.spawn_busy(move |this, ev| async move { this.hold(cmd, ev).await })
            .await;
    }

    /// Hold `direction` for a fixed duration, then stop (CLI `up`/`down`).
    pub async fn hold_for(&self, direction: &str, dur: Duration) {
        if self.conn.lock().await.is_none() {
